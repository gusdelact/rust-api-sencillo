...
//        api.post("greet", |endpoint| {
        ...
        api.put("greet/:name", |endpoint| {
            endpoint.summary("Sends greeting PUT Version");
            endpoint.desc("Use this to talk to yourself");
            endpoint.params(|params| {
                params.req_typed("name", json_dsl::string());
                params.req_typed("greeting", json_dsl::string());
        ...
        api.delete("greet/:name", |endpoint| {
            endpoint.summary("Sends greeting DEL Version");
            endpoint.desc("Use this to talk to yourself");
            endpoint.params(|params| {
                params.req_typed("name", json_dsl::string());
                params.req_typed("greeting", json_dsl::string());
        ...
        api.get("greet/:name", |endpoint| {
            endpoint.summary("Sends greeting GET Version");
            endpoint.desc("Use this to talk to yourself");
            endpoint.params(|params| {
                params.req_typed("name", json_dsl::string());
            });
            endpoint.handle(|client, params| {
                println!("GET greet {}",params);
                client.text(
                    format!("{}",
                        params.find("name").unwrap().to_string())
                )
